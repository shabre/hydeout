...
백분위 수 집계는 다중 숫자 메트릭 집계에 해당한다. percentiles 집계 옵션을 주면 되며 백분위 설정이 따로 가능하다. 아래와 같이 쿼리를 수행하면 된다.
```json
GET /apache-web-log/_search?size=0
{
  "aggs": {
    "bytes_percentiles": {
      "percentiles": {
        "field": "bytes",
        "percents": [10, 20, 30, 40, 50, 60, 70, 80, 90] //입력하지 않으면 default 값으로 출력된다.
      }
    }
  }
}
...
백분위 수 집계 또한 카디널리티 집계와 같이 근사치 계산으로 집계된다. 카디널리티 집계와 마찬가지로 문서들의 집합 크기가 작을수록 정확도가 더 높아지고, 커질수록 점점 더 근사치를 계산하게 된다.

### 백분위 수 랭크 집계
백분위 수 집계가 백분위 수에 해당하는 수치를 파악했다면 백분위 수 랭크 집계는 이와 반대로 특정 수를 입력하면 그 수가 어떤 백분위에 속하는지 알 수 있다.
```json
GET /apache-web-log/_search?size=0
{
  "aggs": {
